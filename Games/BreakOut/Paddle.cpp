...
bool Paddle::Bounce(Ball& ball)
{
    ...
    if(HasCollided(ball.GetBoundingRect(), edge))
    {
        ...
        if(edge.edge == GetEdge(TOP_EDGE).edge)
        {
            ...
            float spotHitOnPaddle = (pointOnEdge.GetX() - edge.edge.GetP0().GetX()) / edgeLength;

            if(((spotHitOnPaddle <= CORNER_BOUNCE_AMOUNT) && ball.GetVelocity().GetX() > 0) ||
               (spotHitOnPaddle >= (1.0f - CORNER_BOUNCE_AMOUNT) && ball.GetVelocity().GetX() < 0))
            {
